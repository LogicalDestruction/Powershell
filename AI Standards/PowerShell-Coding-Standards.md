...
When writing a script, do not ask for every variable before producing code.

Use reasonable placeholder values in the User Configurable Variables section and make them easy to edit.

Prefer this:

```powershell
$ServerName = 'server01.domain.com'
$OutputPath = 'C:\Temp\Service-Check.csv'
$ServiceName = 'Spooler'
...
## Options: Initialize, Verify, and Remediation
...
Do not hardcode passwords, tokens, API keys, or secrets. If a temporary lab/demo requires it, clearly mark it as unsafe for production.
...
Alternatively we might use creds saved in a pwfile
```powershell
...
Use parameters only when the script is intended to be called by automation, a scheduled task, another script, or when the user specifically asks for parameterized input.
...
Use functions when they make the script easier to read or reduce repeated logic that exceeds reuse more than 2 times. (rare)

Functions should not hide the main flow of the script.

A sysadmin should be able to scroll through the script and understand the order of operations without jumping through many functions.
...
Do not default to advanced PowerShell patterns unless they are specifically needed.

Avoid these unless requested or clearly justified:

- Classes
- Custom modules
- Complex parameter sets
- Pipeline-heavy one-liners
- Nested functions
- Scriptblocks passed around like application code
- Complex hashtable-driven frameworks
- Excessive splatting for simple commands
- Background jobs or runspaces
- Pester tests
- CI/CD pipeline assumptions
- JSON/YAML config files

For normal sysadmin scripts, prefer a single readable `.ps1` file with a clear variable block and straightforward top-to-bottom logic.
